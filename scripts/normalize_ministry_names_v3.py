...
def clean_ministry_name(ministry):
    ...
    ministry_str = re.sub(r'\s*[-)]\s*\([^)]*\)\s*:', '', ministry_str)
    # 2. Remove trailing dash
    ministry_str = re.sub(r'\s*-\s*$', '', ministry_str)
    # 3. Remove trailing N followed by numbers and optional parentheses
    ministry_str = re.sub(r'\s*N\d+\)?\s*$', '', ministry_str)
    # 4. Remove trailing parentheses with numbers
    ministry_str = re.sub(r'\s*\(\d+\)\s*$', '', ministry_str)
    # 5. Remove trailing colon
    ministry_str = re.sub(r'\s*:\s*$', '', ministry_str)
    # 6. Remove trailing dash with optional colon
    ministry_str = re.sub(r'\s*-\s*\)?\s*$', '', ministry_str)
    # 7. Remove any remaining trailing parentheses
    ministry_str = re.sub(r'\s*\)\s*$', '', ministry_str)
