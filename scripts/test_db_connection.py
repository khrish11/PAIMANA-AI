...
try:
    ...
    engine = create_engine("postgresql+psycopg2://paimana@127.0.0.1:5432/paimana_ai")
...
try:
    from sqlalchemy import create_engine, text
    engine = create_engine("postgresql+psycopg2://paimana:paimanapass@127.0.0.1:5432/paimana_ai")
    with engine.connect() as conn:
        result = conn.execute(text("SELECT 1"))
