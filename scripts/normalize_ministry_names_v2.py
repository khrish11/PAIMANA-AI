...
def normalize_ministry(ministry):
    ...
    # Remove trailing artifacts aggressively
    # Remove patterns like "-) (", "-) :", "N06000245) (", etc.
    ministry_str = re.sub(r'\s*[-)]\s*\([^)]*\)\s*$', '', ministry_str)
    ministry_str = re.sub(r'\s*[-)]\s*\([^)]*\)\s*:', '', ministry_str)
    ministry_str = re.sub(r'\s*[-)]\s*$', '', ministry_str)
    ministry_str = re.sub(r'\s*[-)]\s*:', '', ministry_str)
    # Remove trailing N followed by numbers and parentheses
    ministry_str = re.sub(r'\s*N\d+\)?\s*$', '', ministry_str)
    # Remove trailing parentheses with numbers
    ...
    # If still not found, return original (cleaned)
    return ministry_str
