...
ministry_normalization = {
    ...
    'Department of water resources-MH': 'Department of Water Resources, River Development & GR',
    'Department of Water Resources, River Development,': 'Department of Water Resources, River Development & GR',
...
def clean_ministry_name(ministry):
    ...
    # 1. Remove patterns like "N06000245) (", "N22000620) (", etc.
    ministry_str = re.sub(r'\s*N\d+\)\s*\(\s*$', '', ministry_str)
    ministry_str = re.sub(r'\s*N\d+\)\s*\(\s*:', '', ministry_str)
    ministry_str = re.sub(r'\s*N\d+\)\s*\(\s*-', '', ministry_str)
    ...
    # 5. Remove trailing N followed by numbers and optional parentheses
    ministry_str = re.sub(r'\s*N\d+\)?\s*$', '', ministry_str)
    
    # 6. Remove trailing parentheses with numbers
    ministry_str = re.sub(r'\s*\(\d+\)\s*$', '', ministry_str)
    
    # 7. Remove trailing colon
    ministry_str = re.sub(r'\s*:\s*$', '', ministry_str)
    
    # 8. Remove any remaining trailing parentheses
    ministry_str = re.sub(r'\s*\)\s*$', '', ministry_str)
    
    # 9. Remove trailing dash with optional colon
    ministry_str = re.sub(r'\s*-\s*\)?\s*$', '', ministry_str)
    
    # 10. Remove state codes like "-JH", "-MH", "-PB"
    ministry_str = re.sub(r'\s*-[A-Z]{2}\s*$', '', ministry_str)
    
    # 11. Remove trailing "II" or "I"
    ministry_str = re.sub(r'\s*[IV]+\s*$', '', ministry_str)
    
    # 12. Remove trailing "N"
    ministry_str = re.sub(r'\s*N\s*$', '', ministry_str)
    
    # 13. Remove trailing comma
    ministry_str = re.sub(r'\s*,\s*$', '', ministry_str)
